...
// Features: Inclusion Proofs (SPV), Memory Optimization, Deterministic Sorting.
...
/// Indicates whether the sibling hash is to the left or right of the current node.
...
impl MerkleTree {
    ...
    pub fn generate_proof(leaves: &[String], target_index: usize) -> Option<MerkleProof> {
        ...
        let target_leaf = if target_index < sorted_leaves.len() {
            &sorted_leaves[target_index]
        } else {
            return None;
        };
    ...
    /// Returns the standard hash for an empty tree.
    pub fn empty_root() -> String {
        hex::encode(Sha256::digest("EMPTY_MERKLE_ROOT"))
    }
}
...
mod tests {
    ...
    fn test_single_leaf() {
        ...
        // Proof for single leaf should be empty steps
    ...
    fn test_odd_leaves_duplication() {
        ...
        // Verify that re-ordering doesn't change root (determinism)
